...
async fn main() -> std::io::Result<()> {
    let required_public_key = PublicKey::from_bytes_hex(
        &std::env::var("BISCUIT_PUBLIC_KEY")
            .expect("Missing BISCUIT_PUBLIC_KEY environment variable. You can fix it by using the following command to run the example: BISCUIT_PUBLIC_KEY=2d6a07768e5768192870f91a6949cd09ce49865f2e2eb1241369c300ee7cc21f cargo run --example configuration"), biscuit_auth::Algorithm::Ed25519,
    )
    ...
    let optional_public_key =
        std::env::var("BISCUIT_OPTIONAL_PUBLIC_KEY")
            ...
            .and_then(|bytes_hex| {
                Some(
                    PublicKey::from_bytes_hex(&bytes_hex, biscuit_auth::Algorithm::Ed25519)
